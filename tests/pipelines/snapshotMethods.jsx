...
export default async function run({ test, assert }) {
    ...
    await test("toSnapshot: returns react-test-renderer JSON tree", async () => {
    ...
    await test("toSnapshot: returns tree for complex component", async () => {
        const tree = await CTGReactTest.toSnapshot(<Counter />);
        assert(tree !== null, "tree is not null");
        assert(tree.type === "div", "root is div");
        assert(Array.isArray(tree.children), "has children");
    });

    await test("toSnapshot: fresh render has no internal state", async () => {
        const a = await CTGReactTest.toSnapshot(<Counter initial={0} />);
        const b = await CTGReactTest.toSnapshot(<Counter initial={0} />);
        assert(JSON.stringify(a) === JSON.stringify(b), "fresh renders are identical");
    });

    // ── compareSnapshot ─────────────────────────────────────────

    await test("compareSnapshot: returns true for identical components", async () => {
    ...
    await test("compareSnapshot: returns false for different props", async () => {
        const match = await CTGReactTest.compareSnapshot(
            <Greeting name="World" />,
            <Greeting name="Other" />
        );
    ...
    await test("compareSnapshot: returns false for different components", async () => {
        const match = await CTGReactTest.compareSnapshot(
            <Greeting name="World" />,
            <Counter initial={0} />
        );
    ...
    await test("compareSnapshot: built on diffSnapshot", async () => {
    ...
    await test("diffSnapshot: returns empty array for identical components", async () => {
        const diffs = await CTGReactTest.diffSnapshot(
            <Greeting name="World" />,
            <Greeting name="World" />
        );
    ...
    await test("diffSnapshot: returns differences for different props", async () => {
        const diffs = await CTGReactTest.diffSnapshot(
            <Greeting name="A" />,
            <Greeting name="B" />
        );
    ...
    await test("diffSnapshot: diff entries have path, expected, actual", async () => {
        const diffs = await CTGReactTest.diffSnapshot(
            <Greeting name="A" />,
            <Greeting name="B" />
        );
    ...
    await test("diffSnapshot: returns differences for different components", async () => {
        const diffs = await CTGReactTest.diffSnapshot(
            <Greeting name="World" />,
            <Counter initial={0} />
        );
